...
class WikiMultiHopQAEvalDataset(Dataset):
    def __init__(self, data_paths, max_samples=None):
        ...
        for x in self.samples:
            ...
            normalized.append({"question": q, "answer": str(a), "aliases": aliases})
...
def search(query: str):
    ...
    #     "return_scores": True
    # }
    # raw_results = requests.post("http://127.0.0.1:8000/retrieve", json=payload).json()['result'][0]
...
def evaluate_hf(
    ...
    with torch.no_grad():
        ...
        for batch in iterator:
            ...
            # Evaluate
            all_aliases = batch.get("aliases", [[] for _ in questions])
            for question, gold_answer, gen_text, gen_time, aliases in zip(questions, gold_answers, gen_texts, gen_times, all_aliases):
                ...
                if not is_correct and aliases:
                    for alias in aliases:
                        if normalize_answer(predicted_answer) == normalize_answer(str(alias)):
                            is_correct = True
                            break
