...
class SearchR1ProtocolTests(unittest.TestCase):
    def test_corrected_tokenizer_snapshot_is_passed_to_vllm(self):
        calls = {}
        ...
        class FakeAutoTokenizer:
            ...
            def from_pretrained(model_path, **kwargs):
                ...
                calls["kwargs"] = kwargs
                return FakeLoadedTokenizer()

        fake_transformers = types.SimpleNamespace(AutoTokenizer=FakeAutoTokenizer)
        with tempfile.TemporaryDirectory() as temp_dir, mock.patch.dict(
            sys.modules, {"transformers": fake_transformers}
        ), mock.patch.dict(os.environ, {"TMPDIR": temp_dir}):
            tokenizer, saved_path = search_r1.load_corrected_tokenizer("checkpoint")

        self.assertIsInstance(tokenizer, FakeLoadedTokenizer)
        self.assertEqual(calls["model_path"], "checkpoint")
        self.assertEqual(calls["kwargs"], {"fix_mistral_regex": True})
        self.assertEqual(Path(saved_path), calls["saved"])
