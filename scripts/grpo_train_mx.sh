...
OUTPUT_DIR="${SAVE_DIR}/${SAVE_VERSION}-${MODEL_PATH##*/}-${LOSS_TYPE}-g${NUM_GENERATIONS}-ndb${NUM_DB_ROLLOUTS}-lr${LEARNING_RATE}-bs${PER_DEVICE_TRAIN_BATCH_SIZE}-s${GRADIENT_ACCUMULATION_STEPS}-b${BETA}-ep${NUM_TRAIN_EPOCHS}-n${TRAIN_SIZE}-${REWARD_FUNC}"
if [ -n "${TWO_PHASE}" ]; then
