...
elif [ "${DATASET}" = "mquake-cf5334" ]; then
    if [ "${THRESHOLD}" = "1.0" ]; then
        # DATASET_PATH=/share/j_sun/lz586/lmlm_multihop/sft_data/CF_q300_r10.json
        DATASET_PATH=/share/j_sun/lz586/lmlm_multihop/sft_data/CF-q6334-r10_CF_q700_r10.json
    else
        echo "Invalid threshold: ${THRESHOLD}"
        exit 1
    fi
elif [ "${DATASET}" = "mquake-real-debug" ]; then
    if [ "${THRESHOLD}" = "1.0" ]; then
        DATASET_PATH=/share/j_sun/lz586/lmlm_multihop/sft_data/REAL_q3000_debug.json
    else
        echo "Invalid threshold: ${THRESHOLD}"
        exit 1
    fi
elif [ "${DATASET}" = "mquake-cf-debug" ]; then
    if [ "${THRESHOLD}" = "1.0" ]; then
        DATASET_PATH=/share/j_sun/lz586/lmlm_multihop/sft_data/CF_q300_r10_debug.json
    else
        echo "Invalid threshold: ${THRESHOLD}"
        exit 1
    fi
