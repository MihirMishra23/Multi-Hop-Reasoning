"""Token-level F1 reward used by Linxi's Search-R1 paper runs.

This preserves the scoring path from c30a5d9 so the Search-R1 and KBevo runs
use the same normalized token-overlap F1.
"""
...
def compute_score(data_source, solution_str, ground_truth, extra_info=None):
    ...
    for target in targets:
        gold_tokens = normalize_text(str(target)).split()
        best = max(best, token_f1(pred_tokens, gold_tokens))
