#!/usr/bin/env python3
"""
Local dense retrieval server that speaks verl's batched /retrieve protocol.
...
def retrieve(req: QueryRequest):
    ...
    # A single shared service feeds all Search-R1 evaluation GPUs. Serialize
    # access to the one CUDA encoder/FAISS reader; generation remains parallel.
    with retriever_lock:
        results, scores = retriever.batch_search(
            req.queries,
            num=topk,
            return_score=req.return_scores,
        )
    resp = []
