#!/usr/bin/env python3
"""
Script to pull random questions from MuSiQue and generate responses using GPT-4.
"""
...
def save_paragraphs_to_file(example, question_id):
    """Save the paragraphs for a question to a JSON file."""
    filename = f"musique_paragraphs_{question_id}.json"
    
    # Extract paragraphs from the example
    paragraphs_data = {
    ...
    # MuSiQue dataset has 'paragraphs' field which contains the context
    if 'paragraphs' in example:
        ...
        for para in paragraphs:
            if isinstance(para, dict):
                ...
                paragraphs_data['paragraphs'].append(str(para))
    
    # Save to JSON file
    with open(filename, 'w', encoding='utf-8') as f:
        json.dump(paragraphs_data, f, indent=2, ensure_ascii=False)
    
    print(f"\n✓ Paragraphs saved to: {filename}")
    return filename
...
def main():
    ...
    for i, example in enumerate(examples_to_process, 1):
        ...
        # Save paragraphs to a JSON file for reference
        save_paragraphs_to_file(example, question_id)
